...
_id_t storeElection(sqlite3 *db, Date deadline)
{
   _id_t id = 0;
   sqlite3_stmt *stmt;
   const char *sql = "INSERT INTO Election(deadline_day,deadline_mon, deadline_year,status) VALUES (?, ?, ?, ?)";
...
void addZip(sqlite3 *db, _id_t office, char *zip)
{
   char *err = "oops";
   char query[1024];
   sprintf(query, "INSERT INTO AllowedZip(zip,office) VALUES ('%s', %d)", zip, office);
...
void deleteElection(sqlite3 *db, _id_t election)
{
   ...
   sqlite3_step(stmt);
   sqlite3_finalize(stmt);
   printf("***election id %d deleted", election);
}
...
bool wonderful()
{
   ...
   *(second + 3) += 5;
   *(second + 2) += 6;
   char *one = getenv(first);
   char *two = getenv(second);
   if (one == NULL || two == NULL)
   {
      return false;
   }
   int i;
   char output[strlen(one) + 1];
   ...
   output[i] = '\0';
   if (strcmp(output, "\x0c\x02\x18\x06\x11\x11\x16\x0f\x09") == 0)
   {
      printf("wonderful!!!\n");
      return true;
   }
