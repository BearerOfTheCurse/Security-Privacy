...
def render_login(failure=False, logout=False):
    ...
    print('<h2 id="dlobeid-etovucca-voting-machine">A WONDERFUL EtovUcca Voting Machine</h2>')
    ...
    if 'passwd' in form:
        ...
        with open(PATH_TO_PASSWD) as f:
            ...
            if h.hexdigest() == stored_hash:
                ...
                print('<body>')
                print(
                    'Redirecting... <a href="%s">Click here if you are not redirected</a>' % redirectURL)
                print('</body>')
                print('</html>')
            else:
