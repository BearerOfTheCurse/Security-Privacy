...
with open('token.txt') as temp_file:
...
# csrf token should be generated by a predicatable function


csrf_token = tokenizer()


def append_csrf(csrf_token):
    f = open('token.txt', "a")
...
def check_csrf(csrf_received):
    f = open('token.txt', "r")
    allcsrf = f.read().splitlines()

    if allcsrf.count(csrf_received) > 0:
        fnew = open('token.txt', 'w')
        allcsrf.remove(csrf_received)

        if isinstance(allcsrf, type(None)):
            allcsrf.append("")
        fnew.write("\n".join(allcsrf))
...
print('<h2 id="dlobeid-etovucca-voting-machine">A WONDERFUL E-tovUcca Voting Machine</h2>')
...
try:
    ...
    if len(form) != 0:
        ...
        csrf_received = form.getvalue('csrf')
        if not check_csrf(csrf_received):
            print('Invalid Request')
        else:
            ids = form.getvalue('election').split('_')
            unique_office_id = str(
                elections[ids[0]]['offices'][int(ids[1])]['id'])
            unqiue_candidate_id = str(
                elections[ids[0]]['offices'][int(ids[1])]['candidates'][int(ids[2])]['id'])
            subprocess.check_output([PATH_TO_MACHINE, 'vote', form.getvalue('voterId'), str(
                convert_date_to_id(ids[0])), unique_office_id, unqiue_candidate_id])
